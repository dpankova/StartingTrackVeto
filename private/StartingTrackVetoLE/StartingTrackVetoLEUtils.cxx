        ...
        I3GeometryConstPtr geo, I3VectorOMKeyPtr badDOMs )
{
    ...
    I3ParticleConstPtr fit_0 = frame->Get<I3ParticleConstPtr>(fitName);
    I3Particle* fit  = new I3Particle(); 
    fit->SetPos((*fit_0).GetPos());
    fit->SetTime((*fit_0).GetTime());
    fit->SetDir((*fit_0).GetDir());
    fit->SetEnergy((*fit_0).GetEnergy());
    fit->SetLength((*fit_0).GetLength());
    fit->SetType(I3Particle::unknown);
    fit->SetShape(I3Particle::InfiniteTrack);
    fit->SetLocationType((*fit_0).GetLocationType());
    fit->SetFitStatus(I3Particle::OK);
    ...
    I3MapKeyVectorDouble probtimes;
    I3MapKeyVectorDouble Qs;
    I3MapKeyVectorDouble Ts;
    I3MapKeyDouble mintimes;
    I3MapKeyDouble maxtimes;
	...
	Ts[omkey] = ts;
	    ...
	    probtimes[omkey]=sumQuantilesPerTime;
	  ...
	  // printf("%f",sumQuantilesPerTime);
        ...
        for ( unsigned int i = 0; i < timeEdges.size(); i++ )
        {
	  //printf("sum_quantiles_pertime=%f maxQuantilePerTime=%f",sumQuantilesPerTime[i],maxQuantilePerTime);
	...
	//printf("minTime=%f maxTime=%f",minTime,maxTime);
	mintimes[omkey] = minTime;
	maxtimes[omkey] = maxTime;  
        //If for some reason you encounter a nan value, indicating something bad, record 0 and keep moving
    ...
    std::string nameSufixes[19] = { "allObsQs","qs","ts","minTimes","maxTimes","probtimes",
    ...
    frame -> Put( names[nameIdx], boost::make_shared<I3MapKeyVectorDouble>(Qs) ); nameIdx++;
    frame -> Put( names[nameIdx], boost::make_shared<I3MapKeyVectorDouble>(Ts) ); nameIdx++;
    frame -> Put( names[nameIdx], boost::make_shared<I3MapKeyDouble>(mintimes) ); nameIdx++;
    frame -> Put( names[nameIdx], boost::make_shared<I3MapKeyDouble>(maxtimes) ); nameIdx++;
    frame -> Put( names[nameIdx], boost::make_shared<I3MapKeyVectorDouble>(probtimes) ); nameIdx++;
...
void FindNormalization( I3FramePtr frame, std::string pulsesName,
        std::string fitName, int ls, double tol,
			int iterations, bool supress_stoch, bool cascade, bool norm )
{
	...
	//	printf("minCherdat=%f",minCherdat);
